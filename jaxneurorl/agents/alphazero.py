...
class AlphaZeroLossFn(vbb.RecurrentLossFn):
  ...
  def loss_fn(self, data, online_preds, target_preds, batch_index, steps: int):
    ...
    # [T] --> [T, A]
    random_policy_mask = jnp.broadcast_to(is_last[:, None], policy_target.shape)
    ...
    # Value loss
    ################
    target_net_values = self.discretizer.logits_to_scalar(target_preds.value_logits)
    ...
    value_ce = jax.vmap(rlax.categorical_cross_entropy)(
      value_probs_target, online_preds.value_logits[:num_v_preds]
    )
...
def make_agent(
  ...
  agent = AlphaZeroAgent(
    ...
    observation_encoder=MLP(hidden_dim=config["AGENT_HIDDEN_DIM"], num_layers=1),
    rnn=vbb.ScannedRNN(hidden_dim=config["AGENT_HIDDEN_DIM"], unroll_output_state=True),
...
def make_loss_fn_class(config, discretizer: utils.Discretizer) -> vbb.RecurrentLossFn:
  return functools.partial(
    AlphaZeroLossFn, discretizer=discretizer, discount=config["GAMMA"]
  )
...
def make_actor(
  ...
  def actor_step(
    ...
    root = mctx.RootFnOutput(
      prior_logits=preds.policy_logits, value=value, embedding=preds.state
    )

    # will vmap over mcts
    # mcts excepts []
    # [B,...] --> [B, 1, ...]
    root = jax.tree_util.tree_map(lambda x: x[:, None], root)
    ...
    # [B, 1, ...] --> [B, ...]
    mcts_outputs = jax.tree_util.tree_map(lambda x: x[:, 0], mcts_outputs)
  ...
  return vbb.Actor(
    train_step=actor_step, eval_step=functools.partial(actor_step, evaluation=True)
  )
...
def learner_log_extra(
  ...
  def callback(d):
    ...
    nplots = 4
    fig, (ax1, ax2, ax3, ax4) = plt.subplots(nplots, 1, figsize=(5, 3 * nplots))
...
def make_train_preloaded(config, test_env_params=None):
  ...
  def make_logger(
    ...
    return loggers.Logger(
      ...
      learner_log_extra=functools.partial(learner_log_extra, config=config),
    )

  return functools.partial(
    vbb.make_train,
    make_agent=functools.partial(make_agent, test_env_params=test_env_params),
    make_optimizer=make_optimizer,
    make_loss_fn_class=functools.partial(make_loss_fn_class, discretizer=discretizer),
    make_actor=functools.partial(
      make_actor, discretizer=discretizer, mcts_policy=mcts_policy
    ),
