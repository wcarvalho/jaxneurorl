...
ActorStepFn = Callable[
  [TrainState, AgentState, TimeStep, jax.random.PRNGKey],
...
class PQNLossFn:
  ...
  def error(self, data, preds, steps, **kwargs):
    ...
    selector_actions = jnp.argmax(self.extract_q(preds), axis=-1)  # [T+1, B]
...
class RnnAgent(nn.Module):
  ...
  def unroll(
    self, rnn_state, xs: TimeStep, rng: jax.random.PRNGKey, train: bool = True
  ):
    # rnn_state: [B]
    # xs: [T, B]
...
def learn_step(
  ...
  (_, (updates, metrics)), grads = jax.value_and_grad(
    ...
    has_aux=True,
    allow_int=True,
  )(train_state.params, learn_trajectory, _rng, train_state.n_grad_steps)
...
def make_actor(config: dict, agent: Agent, rng: jax.random.PRNGKey) -> Actor:
  ...
  def actor_step(
    ...
    preds, agent_state = agent.apply(variables, agent_state, timestep, rng)
...
def make_train(
  ...
  config["NUM_UPDATES"] = int(config["TOTAL_TIMESTEPS"] // config["BATCH_SIZE"])
  ...
  def train(rng: jax.random.PRNGKey):
    ...
    def _train_step(old_runner_state: RunnerState, unused):
      ...
      buffer_state = buffer_state.replace(experience=buffer_traj_batch)
