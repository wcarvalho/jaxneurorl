  ...
  def test_q_learning_lambda_td(self):
    ...
    for loss_fn in (losses.q_learning_lambda_td, losses.q_learning_n_step_td):
      for t, terminal, discount in pairs:
        ...
        q = jnp.array([[0, 0, 0], [0, 0, 0]], dtype=jnp.float32).T  # make time first
        ...
        if t == "terminal":
          ...
          chex.assert_trees_all_close(target_mt1[:2], target_mt1[-2:], atol=atol)
          chex.assert_trees_all_close(target_mt1[2], 0, atol=atol)
        elif t == "truncation":
          chex.assert_trees_all_close(target_mt1[:2], target_mt1[-2:], atol=atol)
