...
#def process_episode(key, gds, gd_infos, example_timestep):
#    red = raw_episode_data = gds[key]
...
#    reaction_times = jnp.asarray([compute_reaction_time(datum) for datum in red])
...
#    return episode_idx, episode_data, info

#def parallel_make_episode_data(data: List[dict], example_timestep: multitask_env.TimeStep):
#    gds, gd_infos = separate_data_by_block_stage(data)
    
#    with multiprocessing.Pool() as pool:
#        results = pool.map(
#            partial(process_episode, gds=gds, gd_infos=gd_infos, example_timestep=example_timestep),
#            gds.keys()
#        )
...
#    for episode_idx, ep_data, info in results:
#        episode_data[episode_idx] = ep_data
#        episode_info[episode_idx] = info
