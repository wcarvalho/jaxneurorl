...
class MazeRenderer(object):
    ...
    @partial(jax.jit, static_argnums=(0,))
    def render_state(self, env_state: EnvState, env_params: EnvParams, highlight=False) -> chex.Array:
        ...
        if self.render_border:
            cells = jnp.pad(jnp.where(env_state.wall_map, 1, 0),
                            1, mode="constant", constant_values=True)
        else:
            cells = jnp.where(env_state.wall_map, 1, 0)
        cells = jnp.where(cells, 1, 0)
        cells = cells.at[agent_pos[1], agent_pos[0]].set(
            3 + env_state.agent_dir)
        cells = cells.at[goal_pos[1], goal_pos[0]].set(2)

        img = self._atlas[cells].transpose(
            0, 2, 1, 3, 4).reshape(height_px, width_px, 3)
        ...
        min_x = jnp.minimum(jnp.maximum(
            min_bound[0], 0), env_state.wall_map.shape[0] - 1 + 2*self.render_border)
        min_y = jnp.minimum(jnp.maximum(
            min_bound[1], 0), env_state.wall_map.shape[1] - 1 + 2*self.render_border)
        max_x = jnp.minimum(jnp.maximum(
            min_bound[0]+agent_view_size, 0), env_state.wall_map.shape[0] + 2*self.render_border)
        max_y = jnp.minimum(jnp.maximum(
            min_bound[1]+agent_view_size, 0), env_state.wall_map.shape[1] + 2*self.render_border)
        ...
        mask = \
            ...
            ((all_pos // ncols) >= min_y) & \
            ((all_pos // ncols) < max_y)
        mask = jnp.kron(mask.reshape(nrows, ncols), jnp.ones(
            (self.tile_size, self.tile_size)))[..., None]

        if highlight:
            highlight_img = (img + 0.3 * (255 - img)
                            ).astype(jnp.uint8).clip(0, 255)
        else:
            highlight_img = img
...
def _make_tile_atlas(tile_size):
    ...
    atlas[0] = add_border(
        np.tile([0, 0, 0], (tile_size, tile_size, 1)))  # empty
...
def place_arrows_on_image(image, actions, env_state, offset=8, ax=None):
    ...
    for (y, x), action in actions.items():
        ...
        ax.arrow(center_x, center_y, dx, dy, head_width=scale_x /
                 10, head_length=scale_y/10, fc='g', ec='g')
